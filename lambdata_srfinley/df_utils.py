...
def break_date(df, col):
    """
    Takes a dataframe and the name of a column in it
    that consists of date-formatted strings.
    Returns a copy of that dataframe, but without the original dates column,
    and including the column of datetimes, as well as
    three columns of ints for year, month, and day.
    """
    df2 = df.copy()
    date_col = 'dates'
    if col == 'dates':
